...
namespace
{
    ...
    void parseShaderForMerging( const std::string& source, unsigned& version, bool& compatibility, HeaderMap& headers, std::stringstream& body )
    {
        // break into lines:
        StringVector lines;
                             ...
                             osg::Program*                            program )
    {
        ...
        if ( s_mergeShaders )
        {
            ...
            for( VirtualProgram::ShaderVector::const_iterator i = shaders.begin(); i != shaders.end(); ++i )
            {
                ...
                {
                    parseShaderForMerging( s->getShaderSource(), vertVersion, vertCompatibility, vertHeaders, vertBody );
                }
                else if ( s->getType() == osg::Shader::FRAGMENT )
                {
                    parseShaderForMerging( s->getShaderSource(), fragVersion, fragCompatibility, fragHeaders, fragBody );
                }
            }
